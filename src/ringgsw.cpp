 ...
 * Constructor
 */
ringgsw::ringgsw(long N, long n, long m, ZZ q, long Q){
  this->N = N;
  this->n = n;
  this->m = m;
  this->q = q;
 ...
 */ 
Mat<ZZ_p> ringgsw::Encrypt(ZZ_p m){
  Mat<ZZ_p> c;
  Mat<ZZ_p> r;
 ...
 */ 
ZZ_p ringgsw::Decrypt(Mat<ZZ_p> c){
  int i;
  ZZ_p res;
  ZZ_p a;
  Vec<ZZ_p> columnI;
